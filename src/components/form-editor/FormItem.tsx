...
import { MoreHorizontal, Plus, X, Pencil, Trash2, Type, AlignLeft, Circle, ChevronDownSquare } from 'lucide-react';
...
export default function FormItemCard({ item, errorId }: Props) {
  const { updateItem, deleteItem, addOption, updateOption, deleteOption } = useFormContext();
      ...
      <div className="px-4 pb-4 space-y-3">
        ...
        <div>
          <input
            ...
            className={`w-full text-sm font-medium text-stone-800 bg-transparent border-b pb-1 outline-none transition-colors placeholder:text-stone-300 ${
          ...
          />
          {errorId === `label-${item.id}` && (
            <p className="text-xs text-red-500 mt-1">질문을 입력해주세요</p>
          )}
        </div>
